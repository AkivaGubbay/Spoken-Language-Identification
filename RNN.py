...
def train_neural_network(x):
    ...
    with tf.Session() as sess:
        ...
        # print('prediction: ', correct)    # My code

        accuracy = tf.reduce_mean(tf.cast(correct, 'float'))
        # print('accuracy: ', accuracy)

        print('Accuracy:',
              accuracy.eval({x: epoch_x.reshape((-1, n_chunks, chunk_size)), y: epoch_y}))  # accuracy.eval({x: mnist.test.images.reshape((-1, n_chunks, chunk_size)), y: mnist.test.labels}))
